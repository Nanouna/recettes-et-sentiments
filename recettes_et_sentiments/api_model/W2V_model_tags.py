...
def train_word2vec(tags, vector_size=100, window=5, min_count=1, workers=4):
    return Word2Vec(sentences=tags, vector_size=vector_size, window=window, min_count=min_count, workers=workers)
...
def preprocess_data(data, vector_size=100, window=5, min_count=1, workers=4):
    word2vec_model = train_word2vec(data['tags'], vector_size, window, min_count, workers)
    data['tag_vectors'] = embedding(word2vec_model, data['tags'].tolist(), vector_size)
    return data, word2vec_model

def recommend_recipe(recipes_with_vectors, recipe_id):
    neigh = NearestNeighbors(n_neighbors=2, radius=0.4)
    neigh.fit(np.array(recipes_with_vectors['tag_vectors'].tolist()))
    recipe_index = recipes_with_vectors.index.get_loc(recipe_id)
    distances, indices = neigh.kneighbors([recipes_with_vectors.iloc[recipe_index]['tag_vectors']])

    if len(indices[0]) < 2:
        return None
    return recipes_with_vectors.iloc[indices[0][1]]

data = pd.read_parquet('recettes-et-sentiments/recettes_et_sentiments/api_model/last_preproc_we_hope.parquet')
